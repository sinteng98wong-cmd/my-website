...
import { CheckCircle2, Circle, Banknote, AlertCircle } from "lucide-react";
        ...
        {rows.length === 0 ? (
          ...
          <table className="w-full text-sm">
            <thead className="table-header">
              <tr>
                {["Patient", "Doctor", "Treatment", "Billed", "Patient Paid", "Cash Verified", ""].map(h => (
            ...
            <tbody>
              {rows.map(r => (
                <tr key={r.id} className="table-row">
                  ...
                  <td className="px-4 py-3 text-slate-600 text-sm">{r.doctorName}</td>
            ...
            <tfoot>
              <tr className="border-t-2 border-slate-200 bg-slate-50">
                <td colSpan={3} className="px-4 py-2.5 text-xs font-semibold text-slate-500 uppercase">Day Totals</td>
                <td className="px-4 py-2.5 tabular-nums font-semibold text-slate-800">{RM(totalBilled)}</td>
                <td className="px-4 py-2.5 tabular-nums font-bold text-green-700">{RM(totalCollected)}</td>
                <td colSpan={2} />
              </tr>
            </tfoot>
          </table>
